...
To quantify the chromatic characteristics of floral materials and the overall design scheme, this study adopts the CIE $L^{*}C_{ab}^{*}h_{ab}$ format for the parametric representation of color, using lightness $L^{*}$, chroma $C_{ab}^{*}$, hue angle $h_{ab}$, and color difference $\Delta E_{ab}^{*}$ as the core color metrics \citep{CIE2004Colorimetry,ISOCIE2008Lab,CIE2012ILV}. CIELAB (CIE 1976 $L^{*}a^{*}b^{*}$) is a Uniform Color Space (UCS) recommended by the CIE in 1976, although in practical applications it is only approximately perceptually uniform \citep{Luo2016CIELAB}. Here, $L^{*}$ represents lightness, $a^{*}$ represents redness--greenness, and $b^{*}$ represents yellowness--blueness. CIE $L^{*}C_{ab}^{*}h_{ab}$ is the polar (cylindrical) coordinate form of CIELAB, converting $(a^{*}, b^{*})$ into $C_{ab}^{*}$ (chroma) and $h_{ab}$ (hue angle) to represent perceived chroma and hue, respectively \citep{CIE2004Colorimetry,ISOCIE2008Lab}.

Commonly used color representation methods in academia primarily include color models, color spaces, and color order systems or practical tools used for professional color communication; different representation methods are suitable for different research and application scenarios. These are typically categorized into color models (e.g., RGB, CMYK), color spaces (e.g., CIELAB, CIELUV), and color order systems (e.g., Munsell, NCS), together with color-difference measurement methods based on $\Delta E_{ab}^{*}$ \citep{NemcsicsCaivano2016COS,Munsell1942BookOfColor,HardSivik2001NCS}.

Compared with directly using the rectangular coordinates of $L^{*}a^{*}b^{*}$, $L^{*}C_{ab}^{*}h_{ab}$ maintains the same standard foundation and computability while reorganizing chromaticity information into a ``lightness--chroma--hue'' structure that is closer to the design context, making it more suitable for describing and comparing color scheme features in generative design proposals \citep{Luo2016CIELAB}. Based on the above considerations, this study adopts $L^{*}C_{ab}^{*}h_{ab}$ as the primary quantitative representation space and uses $\Delta E_{ab}^{*}$ to measure color differences between design schemes \citep{ClarkeMcDonaldRigg1984JPC79,CIE1995IndustrialDiff,Melgosa2016CIE94,LuoCuiRigg2001CIEDE2000,Luo2016CIEDE2000}. On this basis, the color parameters used in the analysis are summarized in \autoref{tab:color_metrics}, including $L^{*}$, $C_{ab}^{*}$, $h_{ab}$, $\Delta E_{ab}^{*}$, as well as derived indicators of contrast and harmony (the calculation formulas and implementation details of all metrics are provided in the Appendix).
